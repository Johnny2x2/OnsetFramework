...
AddRemoteEvent("CancelGatherAnimation", function (player) 
    PlayerData[player].isActioned = false
    CallRemoteEvent(player, "MakeNotification", _("gather_cancel", "linear-gradient(to right, #00b09b, #96c93d)"))
    SetPlayerAnimation(player, "STOP")
    CallRemoteEvent(player, "LockControlMove", false)
    SetAttachedItem(player, "hand_r", 0)
    canceled = true
    CallRemoteEvent(player, "CancelGather")
end)
